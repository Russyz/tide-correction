...
E_z = [];
for m = 1
    E = norm(A*[delta_x+ delta1*(m-1); delta_y; delta_z] - o_minus_c/2);    
    grad_E_x =(norm(A*[delta_x + delta1*m; delta_y; delta_z] - o_minus_c/2) - E)/delta1;
    E_x = [E_x;m,grad_E_x]
    grad_E_y = (norm(A*[delta_x; delta_y+delta1*m; delta_z] - o_minus_c/2) - E)/delta1;
    E_y = [E_y;m,grad_E_y]
    grad_E_z = (norm(A*[delta_x; delta_y; delta_z+delta1*m] - o_minus_c/2) - E)/delta1;
    E_z = [E_z;m,grad_E_z]
end
% delta_x = delta_x + E_x(find(abs(E_x(:,2)) == min(abs(E_x(:,2)))),1) * delta1;
% delta_y = delta_y + E_y(find(abs(E_y(:,2)) == min(abs(E_y(:,2)))),1) * delta1;
% delta_z = delta_z + E_z(find(abs(E_z(:,2)) == min(abs(E_z(:,2)))),1) * delta1;
%
lambda = -0.0001;
F = [];
for n =1:100000
    E = E - lambda * (grad_E_x+grad_E_y+grad_E_z);
    F= [F;n, E];
