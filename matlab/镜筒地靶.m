...
for m=1:length(emission_time)
    t1=[];
    w1=find(240e-9<(channel1-t_pre_receive(m))<250e-9);     
    if ~isempty(w1)
    ...
    w2=find(240e-9<(channel2-t_pre_receive(m))<250e-9);
    if ~isempty(w2)
    ...
    w3=find(240e-9<(channel3-t_pre_receive(m))<250e-9);
    if ~isempty(w3)
    ...
    w4=find(240e-9<(channel4-t_pre_receive(m))<250e-9);
    if ~isempty(w4)
...
delay_gt=(1.651*ng-1.651)/c*10^9; %3km地靶大气延时ns
