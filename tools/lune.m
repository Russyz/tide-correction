...
mjd = juliandate(YR,MONTH,DATE)-2400000.5;
up_pre_txyz = [];
down_pre_txyz = [];
for i = 1:length(cpf(:,1))
    if length(cpf(i,:)) > 3
if cpf(i,1) == 10 & cpf(i,2) == 1 & cpf(i,3) == mjd
    up_pre_txyz = [up_pre_txyz;cpf(i,4),cpf(i,6),cpf(i,7),cpf(i,8)];
elseif cpf(i,1) == 10 & cpf(i,2) == 2 & cpf(i,3) == mjd
    down_pre_txyz = [down_pre_txyz;cpf(i,4),cpf(i,6),cpf(i,7),cpf(i,8)];
end
    end
end
% Read SP3 data
...
for i = 1:length(gnt(:,1))
    ...
    for j = 1:4
        ...
        IT2GC = ITRS2GCRS1(YR,MONTH,DATE,hour(i),minu(i),sec(i));
    ...
    for k = 1:4
        ...
        %         syms t_down
        IT2GC = (ITRS2GCRS1(YR,MONTH,DATE,hour(i),minu(i),sec(i)+t_down+t_up));
        coordinate_station_gcrs = IT2GC * coordinate_station_itrs;
...
for i = 1:length(t_up_down(:,1))
    ...
    IT2GC = (ITRS2GCRS1(YR,MONTH,DATE,hour(i),minu(i),sec(i)+t_up_down(i,1)));
    coordinate_station_gcrs = IT2GC * coordinate_station_itrs;
...
%         sp3R = norm(dxyz);
%         t_up0 = (sp3R + corr(i))/c;
%         IT2GC = (ITRS2GCRS1(YR,MONTH,DATE,hour(i),minu(i),sec(i)+t_up_down(i,1)+t_up_down(i,2)));
%         coordinate_station_gcrs = IT2GC * coordinate_station_itrs;
%         dxyz = sp3_IntpXYZ_GCRS - coordinate_station_gcrs';
%         sp3R = norm(dxyz);
